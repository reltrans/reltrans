...
def isco_radius(a, prograde=True):
    """
    Compute the ISCO radius in units of gravitational radius Rg = GM/c^2.

    Parameters
    ----------
    a : float
        Dimensionless spin parameter of the black hole (-1 <= a <= 1).
    prograde : bool, optional
        True for prograde orbit, False for retrograde orbit.
...
def plot_features(ax, xaxis_label = 'Energy [keV]',\
    ...
    ax.set_ylabel(f'{yaxis_label}', fontsize = font)
...
# namefile_GR = f'data_test_GR_quantities_h{(param[0]:.1f)}_a{param[1]:.1f}_gamma{param[6]:.1f}_incl{(param[2])}_rin{(param[3])}_rout{(param[4])}.dat'
# namefile_NW = f'data_test_NW_quantities_h{(param[0]:.1f)}_a{param[1]:.1f}_gamma{param[6]:.1f}_incl{(param[2])}_rin{(param[3])}_rout{(param[4])}.dat'
