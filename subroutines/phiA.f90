...
!-----------------------------------------------------------------------
subroutine newphaseA(nex,nf,earx,ReGrawa,ImGrawa,cosphiA,sinphiA)
  use telematrix
  implicit none
  integer nex,nf
  real earx(0:nex),ReGrawa(nex,nf),ImGrawa(nex,nf)
  real cosphiA(nf),sinphiA(nf)
  ...
  do j = 1,nf
     ...
     call cfold(nex,earx,ReGrawa(:,j),ImGrawa(:,j),ReGtel,ImGtel)
     !Sum up real and imaginary parts
     resum = 0.0
     imsum = 0.0
     do i = Ilo,Ihi
        resum = resum + ReGtel(i)
        imsum = imsum + ImGtel(i)
     end do
     cosphiA(j) =  resum / sqrt( resum**2 + imsum**2 )
     sinphiA(j) = -imsum / sqrt( resum**2 + imsum**2 )     
  end do
  
  return
end subroutine newphaseA
!-----------------------------------------------------------------------
