...
! filenm = '/Users/nai47/Dropbox/mtab/xstar2xspec_out_adam/xout_etable.fits'
...
! /Users/nai47/Dropbox/mtab/xstar2xspec_output_var_fe_adam/xout_etable.fits

!=======================================================================
subroutine mymfe(ear,ne,param,ifl,photar)
! Calculates transmission fraction from a user-defined etable that
! has logxi an Afe as free parameters
  implicit none
  integer ne,ifl,i
  real ear(0:ne),param(5),photar(ne),photer(ne)
  real tau(ne),Nh,logxi,fcov,z,par(3),Nh0,trans(ne),Emax0
  real ear_rest(0:ne),Emin0,Afe
  character (len=255) filenm
  logical firstcall
  save filenm,Emax0,Nh0,firstcall,Emin0
  data firstcall/.true./
...
! Read into mtable parameter array
  par(1) = logxi
  par(2) = Afe
  par(3) = 0.0  !Call mtable with z=0 because the redshift functionality is unreliable
...
! Initialize
  if(firstcall)then
     !Get the name of the etable file
     write(*,*)"Enter name of etable with variable Afe (with full path)"
     read(*,'(a)')filenm
     !Read the largest energy in the grid
     call getEmax(filenm,Emax0)
     !Read the smallest energy in the grid
